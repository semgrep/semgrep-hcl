...
let children_regexps : (string * Run.exp option) list = [
  ...
  Some (
    Alt [|
      Token (Literal "%{if TODO");
      Token (Literal "%{for TODO");
    |];
  );
  "object_end", None;
  "legacy_index",
...
let trans_template_directive ((kind, body) : mt) : CST.template_directive =
  match body with
  | Children v ->
      (match v with
      | Alt (0, v) ->
              ...
              (fun v ->
                Run.repeat
                  (fun v ->
                    ...
                    | Alt (2, v) ->
                        `Temp_dire (
                          trans_template_directive (Run.matcher_token v)
                        )
                    | _ -> assert false
                    )
                  )
